...
//  char *mem; 用不到了去掉，避免编译报错
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    //清除父进程中所有PTE的PTE_W位，并设置PTE_COW位，表示所在页是一个写时复页(多个进程引用同一个物理页)
    //如果该页本身就是不可写的(只读),则不会添加这个标志位
    if(*pte& PTE_W)
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
    if(kama_uvmcheckcowpage(dstva)) //检查每一个页是否是COW页
    {
...
int kama_uvmcowcopy(uint64 va)
{
  ...
  if(((pte = walk(p->pagetable,va,0))) ==0)  //获取虚拟地址的页表项
   ...
   uint64 new = (uint64)kama_kcopy_n_deref((void*)pa);//获取新分配的物理页(如果原本物理页的引用数为1，则获取到的还是原本的物理页)
   if(new == 0)   //内存不足的情况
      return -1;
