...
{
  uint64 n, va0, pa0;
  if(kama_uvmshouldallocate((uint64)dst))
...
void kama_uvmlazyallocate(uint64 va)
{
    ...
    {
      memset(pa,0,PGSIZE);
      if(mappages(p->pagetable,PGROUNDDOWN(va),PGSIZE,(uint64)pa,PTE_W|PTE_R|PTE_X|PTE_U) !=0)  //映射物理地址
      {
