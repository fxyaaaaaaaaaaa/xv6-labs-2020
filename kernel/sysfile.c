...
uint64 sys_mmap(void)
{
    ...
    //读取传入参数
    if(argaddr(0,&addr)<0 || argaddr(1,&sz)<0 || argaddr(2,&prot)<0 || argint(3,&flag)<0 || argfd(4,&fd,&f)<0 || argaddr(5,&offset)<0 ||sz == 0)
        return -1;
    //源文件不可读映射为可读，源文件不可写映射为可写并且要将修改写回源文件
    if((!f->readable && (prot & PROT_READ)) ||  (!f->writable && (prot & PROT_WRITE) && !(flag & MAP_SHARED)))
        return -1;
    sz = PGROUNDUP(sz);
    struct proc* p = myproc();
    struct kama_vma* v = 0;
    ...
    // 遍历查询未被使用的vma并且计算当前已使用的va的最低地址
    for(int i=0 ; i<NVMA ; ++i)
    {
      struct kama_vma* vv = &p->vmas[i];
      if(vv->valid == 0)    //找到空闲vma就保存下来
      {
        if(v == 0)
        {
          v = &p->vmas[i];
...
struct kama_vma* findvma(struct proc * p ,uint64 va)
{
  ...
  {
    struct kama_vma *vv = &p->vmas[i];
    if(vv->valid && va >= vv->vastart && va < vv->vastart + vv->sz)
      return vv;
  }
...
//根据传入的虚拟地址找到属于虚拟地址中的第几个mmap页，分配一块物理空间，将文件数据读取到物理空间中，然后将虚拟空间和物理空间建立映射关系
...
uint64 sys_munmap(void)
{
  ...
  if(v == 0)
      return -1;
  if(addr > v->vastart && addr + sz < v->vastart + v->sz) //释放的区域必须是整个映射页面，而不可以在其中"打洞"
  {
    return -1;
  }
  ...
  int nunmap = sz - (addr_alinged - addr);
  if(nunmap < 0)
    nunmap = 0;
  vmaunmap(p->pagetable,addr_alinged,nunmap,v); //从addr_alinged开始释放nunmap大小的映射
